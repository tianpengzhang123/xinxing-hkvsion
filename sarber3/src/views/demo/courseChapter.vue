<template>
  <basic-container>
    <avue-crud :option="option"
      ...
      <template #menu-left>
        <el-button type="danger"
                   icon="el-icon-delete"
                   plain
                   v-if="permission.courseChapter_delete"
    ...
    <el-drawer :title="`[${courseChapterName}] 配置`" v-model="subVisible" :direction="direction" append-to-body
...
<script>
  import {getList, getDetail, add, update, remove} from "@/api/demo/courseChapter";
  ...
  import option from "@/option/demo/courseChapter";
  ...
  export default {
    data() {
      return {
        ...
        direction: 'rtl',
        courseId: 0,
        courseChapterName: "jxl_course_chapter",
        formSub: {},
        querySub: {},
        loadingSub: true,
        dataSub: [],
    ...
    computed: {
      ...
      permissionList() {
        return {
          addBtn: this.validData(this.permission.courseChapter_add, false),
          viewBtn: this.validData(this.permission.courseChapter_view, false),
          delBtn: this.validData(this.permission.courseChapter_delete, false),
          editBtn: this.validData(this.permission.courseChapter_edit, false)
        };
      },
      ids() {
        let ids = [];
    ...
    methods: {
      ...
      handleExport() {
        let downloadUrl = `/xinxing-demo/courseChapter/export-courseChapter?${this.website.tokenHeader}=${getToken()}`;
        const {
         } = this.query;
        let values = {
        };

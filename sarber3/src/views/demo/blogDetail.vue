    ...
    <avue-crud :option="option"
      ...
      <template #menu-left>
        <el-button type="danger"
                   icon="el-icon-delete"
                   plain
                   v-if="permission.blogDetail_delete"
      ...
      <template #menu="row">
        <el-button type="text"
                   ...
                   @click.stop="handleDataSub(row.row)">配 置
        </el-button>
      </template>
    </avue-crud>
    <el-drawer :title="`[${blogDetailName}] 配置`" v-model="subVisible" :direction="direction" append-to-body
...
<script>
  ...
  import {getList as getListSub, getDetail as getDetailSub, add as addSub, update as updateSub, remove as removeSub} from "@/api/demo/blogDetail";
  import option from "@/option/demo/blog";
  import optionSub from "@/option/demo/blogDetail";
  ...
  export default {
    ...
    computed: {
      ...
      permissionList() {
        return {
          addBtn: this.validData(this.permission.blogDetail_add, false),
          viewBtn: this.validData(this.permission.blogDetail_view, false),
          delBtn: this.validData(this.permission.blogDetail_delete, false),
          editBtn: this.validData(this.permission.blogDetail_edit, false)
        };
      },
      ids() {
        let ids = [];
    ...
    methods: {
      ...
      handleExport() {
        let downloadUrl = `/xinxing-demo/blogDetail/export-blogDetail?${this.website.tokenHeader}=${getToken()}`;
        const {
         } = this.query;
        let values = {
        };
        this.$confirm("是否导出数据?", "提示", {
          ...
          exportBlob(downloadUrl, values).then(res => {
            downloadXls(res.data, `博客详细表${dateNow()}.xlsx`);
            NProgress.done();
          })
        });
      },

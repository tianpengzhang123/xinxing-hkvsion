<template>
  <basic-container>
    <avue-crud :option="option"
      ...
      <template #menu-left>
        <el-button type="danger"
                   icon="el-icon-delete"
                   plain
                   v-if="permission.ljx0903_delete"
    ...
    <el-drawer :title="`[${ljx0903Name}] 配置`" v-model="subVisible" :direction="direction" append-to-body
...
<script>
  import {getList, getDetail, add, update, remove} from "@/api/demo/ljx0903";
  import {getList as getListSub, getDetail as getDetailSub, add as addSub, update as updateSub, remove as removeSub} from "@/api/demo/ljx0904";
  import option from "@/option/demo/ljx0903";
  import optionSub from "@/option/demo/ljx0904";
  ...
  export default {
    ...
    computed: {
      ...
      permissionList() {
        return {
          addBtn: this.validData(this.permission.ljx0903_add, false),
          viewBtn: this.validData(this.permission.ljx0903_view, false),
          delBtn: this.validData(this.permission.ljx0903_delete, false),
          editBtn: this.validData(this.permission.ljx0903_edit, false)
        };
      },
      ids() {
        let ids = [];
    ...
    methods: {
      ...
      handleExport() {
        let downloadUrl = `/xinxing-demo/ljx0903/export-ljx0903?${this.website.tokenHeader}=${getToken()}`;
        const {
            username,
         } = this.query;
        let values = {
        ...
        this.$confirm("是否导出数据?", "提示", {
          ...
          exportBlob(downloadUrl, values).then(res => {
            downloadXls(res.data, `ljx_0903${dateNow()}.xlsx`);
            NProgress.done();
          })
        });
      },
